...
void draw() {
  if (!gaming)    {
    ...
    //background color
    if(boxX > width || boxX < 0 || boxY > height || boxY < 0 || lost)
      background(255,0,0);
    else
...
//           gNotificationManager.notify(-1, vibrateLeft);
         ...
         shape(windshapeLeft, 0, 0,512,512); 
       }
       else{
         //else 
         if (vOn) 
...
//           gNotificationManager.notify(-1, vibrateRight);
         ...
         translate(width/2,height/2);//translate(width-100,height);
         //println("render wind right");  
         shape(windshapeRight, 250, 0,512,512); 
       }
       popMatrix(); 
    }
    else wind = false;
    ...
    if (true){
      ...
      stroke(0,204,51);
      fill(0,204,51);
      text("Goal:" + bonus + "/" + win, -width/2+175, -150, 400, 400);
      stroke(255,153,0);
      fill(255,153,0);
      text("Life:" + health + "/" + life ,-width/2+175, -75, 400, 400);
      popMatrix();
    }
