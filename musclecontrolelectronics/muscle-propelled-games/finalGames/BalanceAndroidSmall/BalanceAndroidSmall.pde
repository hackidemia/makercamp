...
void draw() {
  if (isConfiguring) {
    ...
    text("Click to Connect:", 10, 150);
    text("Enable Vibration: " + enableVibration, 10, 250);
    text("Player ID: " + (2-playerID), 10, 350);
    textSize(30);
    text(info, 50, 450);    
  } else {
...
void resetPowerup()
{
  ...
  myMessage.add(playerID);
  myMessage.add(power.x);
  myMessage.add(power.y);
...
void oscEvent(OscMessage theOscMessage) {
  ...
  if (theOscMessage.get(0).intValue() != playerID) {
    if (theOscMessage.checkAddrPattern("remoteData")) {
      ...
      remoteX = theOscMessage.get(1).floatValue(); 
      remoteY = theOscMessage.get(2).floatValue();
      rAccelerometerX = theOscMessage.get(3).floatValue();
      rAccelerometerY = theOscMessage.get(4).floatValue();
      remoteTargetX = theOscMessage.get(5).intValue();
      remoteTargetY = theOscMessage.get(6).intValue();
